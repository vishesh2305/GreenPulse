...
const MainContent = () => {
        ...
        try {
            ...
            if (response.data) {
                ...
                setResponses(prevResponses => [
                    { id: Date.now(), prompt: userQuery, heading: title, response: aiResponse, image: selectedImage },
                    ...prevResponses
                ]);
            } else {
                setResponses(prevResponses => [
                    { id: Date.now(), prompt: userQuery, heading: "", response: "No Response Received!", image: selectedImage },
                    ...prevResponses
                ]);
            }
        } catch (error) {
            ...
            setResponses(prevResponses => [
                { id: Date.now(), prompt: userQuery, heading: "Error", response: "Failed to Load Response.", image: selectedImage },
                ...prevResponses
            ]);
        }
    ...
    return (
        <div className="h-full place-self-center dark:shadow-greenshadow fixed py-5 lg:mx-auto lg:right-0 w-full  lg:w-5/6 pr-4 shadow-none dark:bg-black dark:text-white bg-white ">
            <div className="my-10 w-full lg:flex lg:flex-col   overflow-y-auto h-[70vh] scrollbar-hidedark:bg-black dark:text-white">
                {responses.map(({ id, prompt, heading, response, image }) => (
                    <div key={id} className="p-4 rounded-md shadow-sm bg-white dark:bg-black dark:text-white">
                        ...
                        {image && <img src={image} alt="Uploaded" className="w-32 h-32 rounded-md mt-2 dark:bg-black dark:text-white" />}
